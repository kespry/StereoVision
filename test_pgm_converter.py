...
#    raise RuntimeError('Required location of source image folder was not supplied.')
    ...
    #assert(type(show_images) is bool)
...
new_files = []
for f in files:
    if f[-13:] == '_stripped.pgm':
        new_files.append(f)
files = new_files

#files = ['/Users/croomjm1/version-control/StereoVisionKespry/1501206316_IR_image_110_modified.pgm']
...
print('Resizing thermal jpgs by 450%.')

#change directory for magick to work
os.chdir(folder)

#resize the jpg in place
os.system('magick mogrify -resize 450% *.jpg')

print('Done resizing thermal jpgs')

#convert pngs to jpg images
print('Converting visual pngs to jpg format.')
os.system('magick mogrify -format .jpg *.png')
